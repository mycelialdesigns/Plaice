# Hinges

<img src="/docs/images/hinge.jpg" alt="A Hinge" width="256" height="256">

Hinges allow Plaice components to rotate around a central axis, and allow many different constructions.

Standard Plaice hinges are designed to align with the relative offset between slides obtained with a corner
when oriented at 90 degrees from straight, and to continue with cell-unit alignment when held straight.

## Printing
<img src="/docs/images/hingesupports.webp" alt="A Hinge, just after printing" width="256" height="256">

Hinges must be printed with supports for the protruding and intruding slides, but the connection between
the hinge halves is print-in-place and requires no supports. 
