...
for top and bottom solid layers. Since all other candidate divisors are smaller than `1.6 mm`, we have to pick `.25 HU` for nice out-of-the-box
slicer behavior. 

However, `1.75 mm` is not a multiple of `.1 mm`, which is the typical layer height employed when printing Plaice components. To accommodate this,
the de facto height of covers is rounded down to `1.7 mm`.
...
for covers to be inserted on adjacent plates, but also to ideally have the result be as visually seamless as possible. Some examples of this
are in Plaice [crosses and tees](/doc/components/CrossesAndTees.md) and [corner stacking caps](/docs/components/CornerStackingCaps.md)
