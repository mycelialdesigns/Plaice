...
Plaice _Sockets_ are chamfered rounded-square voids which allow the insertion of Plaice socket connectors (TODO: cite clips, slides).
They have a total depth of 7mm (1 HU (TODO: cite units)), with 2 x 1.5mm devoted to 45-degree chamfered sections, and with the remaining 4mm consisting of
a rounded 10mm square with a corner fillet radius of 1.6mm. 
...
way that the socket is chamfered. In particular, the radius of the corner fillet does not change along the chamfer, which allows the fillet to be cut more cleanly
...
Additionally, the chamfer length's value of 1.5mm has direct implications for laser cutting, since 1.5mm wood veneers are not terribly hard to come by. While 3d printing
...
Intuitively, since a Plaice plate is just a slab (with slides) with sockets cut out of it, it would make sense that the plate's strength would heavily depend
