...
clipped [connectors](/docs/design/ClipInterface.md) (or nubbed connectors, assuming that M2 support is not relevant to the component in question). To do so,
...
is supposed to be glued on to a Plaice (clipped/nubbed) [cover](/docs/components/Covers.md) with the given width and length dimensions. Out-of-the-box
Plaice components do this routinely when a clipped connector array is involved (E.g: [Gridfinity](/docs/components/GridfinityIntegration.md) and [OpenGrid](/docs/components/OpenGridIntegration.md) baseplates). 
